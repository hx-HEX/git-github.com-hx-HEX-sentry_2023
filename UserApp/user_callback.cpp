...
void User_UART_RX_Callback(UART_HandleTypeDef *huart)
{
	...
	if(RESET != __HAL_UART_GET_FLAG(huart, UART_FLAG_IDLE)) {
		...
		if (huart->Instance == USART1) {
			if ((UART1_DMA_REC_BUFFER_SIZE - huart->hdmarx->Instance->NDTR) 
			== UART1_RECEIVE_DATA_SIZE) {
				//Data Decode
			...
			if ((UART3_DMA_REC_BUFFER_SIZE - huart->hdmarx->Instance->NDTR) 
			== UART3_RECEIVE_DATA_SIZE) {
				// Decode gimbal data
				UART_GimbalDataDecode(uart3_dma_rec_buffer);
				Global::system_monitor.UART3_rx_cnt++;
			}
			HAL_UART_Receive_DMA(huart, uart3_dma_rec_buffer, UART3_DMA_REC_BUFFER_SIZE); 
